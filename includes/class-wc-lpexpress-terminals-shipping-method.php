...
class WC_LPExpress_Terminals_Shipping_Method extends WC_Shipping_Method {
        ...
        if ( !empty( $terminals_json ) ) {
            foreach ($terminals_json as $terminal) {
                $locations[$terminal['addressDetails']['settlementName']][] = (object)array(
                    ...
                    'legacy_id'   => $terminal['code'],
                    'name'        => $terminal['addressDetails']['addressNotes'],
                    'fullAddress' => $terminal['addressText'],
                    'zipcode'     => $terminal['addressDetails']['postCode'],
                    'city'        => $terminal['addressDetails']['settlementName'],
                    'street'      => $terminal['addressDetails']['streetName'],
                    'house'       => $terminal['addressDetails']['houseNumber'],
    ...
    public function save_terminals_cache( $terminals ) {
        ...
        set_transient( $this->id . '_cache_v2', $terminals, 86400 );
    }
    ...
    public function validate_user_selected_terminal( $posted ) {
        ...
        // Skip validation if any errors already sent
        if ( $this->error_notices_sent() ) {
            return;
        }

        // Check if all products in cart are virtual and stop validation if yes
        if ( $this->cart_has_only_virtual_products() ) {
            return;
        }
    ...
    /**
     * Checks if all products in cart are virtual.
     *
     * @return bool
     */
    protected function cart_has_only_virtual_products() {
        global $woocommerce;
        ...
        return ( count($products) == $virtual_products );
    }

    /**
     * Checks if any error messages already sent to the user.
     * Helps to prevent duplicated error messages.
     *
     * @return bool
     */
    protected function error_notices_sent() {
        $notices = WC()->session->get( 'wc_notices', array());

        return ( isset( $notices['error'] ) && ! empty( $notices['error'] ) );
    }
}
